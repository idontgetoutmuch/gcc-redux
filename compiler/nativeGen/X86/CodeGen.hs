...
sse2NegCode w x = do
  ...
  let
    const = case fmt of
      ...
      x@(II8)  -> panic $ "sse2NegCode: " ++ show x
      x@(II16) -> panic $ "sse2NegCode: " ++ show x
      x@(II32) -> panic $ "sse2NegCode: " ++ show x
      x@(II64) -> panic $ "sse2NegCode: " ++ show x
      x@(FF80) -> panic $ "sse2NegCode: " ++ show x
  Amode amode amode_code <- memConstant (widthInBytes w) const
  tmp <- getNewRegNat fmt
  let
    code dst = x_code dst `appOL` amode_code `appOL` toOL [
